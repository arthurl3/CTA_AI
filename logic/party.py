...
class Party:
    ...
    def play_card(self, card, i_node):
        if card.field:
            ...
            self.board_datas.add_node(i_node, data=node)
            ...
            if self.current_player_control_leader():
                ...
                for i_n in self.board_datas.nodes:
                    ...
                    if self.board_datas.nodes[i_n]['data']['leader']:
                        if self.board_datas.nodes[i_n]['data']['original_owner'] == self.board_datas.current_player:
                            self.enable_leader()
    ...
    def get_power_boost_with_affinities_and_trinities(self, i_node):
        allies, _ = self.get_allies_and_ennemies(i_node)
        node = self.board_datas.nodes[i_node]['data']
        # Verifier trinités et affinités proches (ensemble car ne peut contenir qu'une seule fois une tri ou une aff)
        affinities = set()
        trinities = set()
        i = 0
